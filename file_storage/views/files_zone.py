...
from ..models import File, User, Directory, Member, Link
...
def access_required(func):
    ...
    def decorated_view(user, directory, *args, **kwargs):
        ...
        if back_dir:
            while back_dir.holder_id is not None:
                if not check_for_access(back_dir.id):
        ...
        if request.method == 'POST':
            ...
            if actual_dir.check_password(pass_to_check):
                ...
                try:
                    ...
                    return render_template('files_zone/dir_password.html', dirname = actual_dir.name,
                                           user = owner.username, back_dir = back_dir)
                return redirect(url_for('my_files', user = user, directory = actual_dir.name))
            print("Incorrect")
        return render_template('files_zone/dir_password.html', dirname = actual_dir.name, user = owner.username,
                               back_dir = back_dir)
...
def my_files(user, directory):
    if current_user.is_authenticated:
        ...
        return render_template('files_zone/files.html', dirs = dir_list, files = file_list, current_dir = actual_dir,
                               back_dir = back_dir, user = dir_owner.username)
