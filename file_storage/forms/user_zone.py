...
PASS_MIN_L = 6
PASS_MAX_L = 16
PASS_LENGHT_MESSAGE = "Hasło powinno zawierać od %s do %s znaków" % (str(PASS_MAX_L), str(PASS_MAX_L))
...
class LoginForm(Form):
    username = StringField('name', validators = {DataRequired(), Length(USER_MIN_L, USER_MAX_L, USER_LENGHT_MESSAGE)})
    password = PasswordField('pass', validators = [DataRequired(), Length(PASS_MIN_L, PASS_MAX_L, PASS_LENGHT_MESSAGE)])


class ChangeEmailForm(Form):
    email = EmailField('newEmail', validators = [DataRequired()])
    password = PasswordField('pass', validators = [Length(PASS_MIN_L, PASS_MAX_L, PASS_LENGHT_MESSAGE)])
